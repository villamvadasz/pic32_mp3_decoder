...
void do_mmc_sdcard_microchip(void) {
...
)
{
	...
	BYTE x = 0;
	for (x = 0; x < count; x++) {
		if (MDD_SDSPI_SectorRead(sector + x, buff + (x * MEDIA_BLOCK_SIZE)) == 0) {
			result = RES_ERROR;
		}
	}
	return result;
...
)
{
	...
	for (x = 0; x < count; x++) {
		...
		if (MDD_SDSPI_SectorWrite(sector + x, ptr, 1) == 0) {
			result = RES_ERROR;
		}
	}
	return result;
...
)
{
	...
	switch (ctrl) {
		...
		case GET_SECTOR_COUNT :	/* Get number of sectors on the disk (WORD) */
			*(DWORD*)ptr = MDD_SDSPI_ReadCapacity() / MDD_SDSPI_ReadSectorSize();
			res = RES_OK;
			break;

		case GET_SECTOR_SIZE :	/* Get sectors on the disk (WORD) */
			*(DWORD*)ptr = MDD_SDSPI_ReadSectorSize();
			res = RES_OK;
			break;
...
void SDcard_disk_timerproc (void)
{
