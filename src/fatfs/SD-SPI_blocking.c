...
#include "tmr.h"
    ...
    #include "spi2.h"
    #define SPICalculateBRG SPI2CalculateBRG
    #define readWriteSPI readWriteSPI2
    #define spi_reconfigure spi2_reconfigure
#endif
#ifdef FATFS_USE_SPI3
    #include "spi3.h"
    #define SPICalculateBRG SPI3CalculateBRG
    #define readWriteSPI readWriteSPI3
    #define spi_reconfigure spi3_reconfigure
#endif
#ifdef FATFS_USE_SPI4
    #include "spi4.h"
    #define SPICalculateBRG SPI4CalculateBRG
    #define readWriteSPI readWriteSPI4
    #define spi_reconfigure spi4_reconfigure
#endif
#ifdef FATFS_USE_SPI_SW
    #include "spi_sw.h"
    #define SPICalculateBRG SPI_SW_CalculateBRG
    #define readWriteSPI readWriteSPI_SW
...
void init_sd_spi(void) {
	...
	SDSPI_BRG_LOW = SPICalculateBRG(SPI_FREQUENCY_SLOW);
	SDSPI_BRG_HIGH = SPICalculateBRG(SPI_FREQUENCY);
	
	removeTimer(&do_MDD_SDSPI_MediaInitializeTimer);
	addTimer(&do_MDD_SDSPI_MediaInitializeTimer);
}
...
void isr_sd_spi_1ms(void) {
	static uint8 sd_spi_100msCnt = 0;
	sd_spi_100msCnt++;
	if (sd_spi_100msCnt >= 100) {
		sd_spi_100msCnt = 0;
		do_sd_spi_100ms = 1;
	}
...
//#define MEDIA_SOFT_DETECT
	...
	if (SPIENABLE == 0)
	{
		...
		
		{
			...
			spi_reconfigure(SMP, CKP, CKE, BRG);
		}		
...
BYTE MDD_SDSPI_SectorRead(DWORD sector_addr, BYTE* buffer)
{
    ...
    while(1)
    {
        ...
        {
			if (diskEncryptState) {
				aes_decrypt(encryptBuffer, 512, buffer, &tempOutSize);
			}
            return TRUE;
        }
        ...
        case ASYNC_READ_NEW_PACKET_READY:
            ...
            if(ioInfo.dwBytesRemaining != 0x00000000)
            {
            	...
            	#if defined __C30__ || defined __C32__
                {
                    ...
                    readWriteSPI(NULL, localPointer, localCounter);
                }    
            	#endif   
    ...
    switch(info->bStateVariable)
    {
        ...
        case ASYNC_WRITE_TRANSMIT_PACKET:
        	...
        	#if defined __C30__ || defined __C32__
            {
                ...
                BYTE* localPointer = ioInfo.pBuffer;    
                WORD localCounter = ioInfo.wNumBytes;
				readWriteSPI(localPointer, NULL, localCounter);        
            }                	    
            #endif
...
static unsigned char ReadWriteSPIM( unsigned char data_out ) {
	...
	readWriteSPI(bufferOut, bufferIn, size);
	return ( bufferIn[0] );
}
...
BYTE MDD_SDSPI_SectorWrite(DWORD sector_addr, BYTE* buffer, BYTE allowWriteToZero)
{
	...
	if (diskEncryptState) {
		aes_encrypt(buffer, 512, encryptBuffer, &tempOutSize);
	}
	{
		unsigned char SMP = 1;
		unsigned char CKP = 1;
...
MEDIA_INFORMATION *  MDD_SDSPI_MediaInitialize(void)
{
    ...
    if(timeout == 0)
    {
        ...
        response = SendMMCCmd(STOP_TRANSMISSION, 0x0);    //Blocks until SD card signals non-busy
