...
static volatile UINT Timer2;		/* 1000Hz decrement timer */
static UINT CardType;

unsigned char bufferOutMMC = 0;
unsigned char bufferInMMC = 0;
...
static BYTE xchg_spi (BYTE dat) {
	bufferOutMMC = dat;
	readWriteSPI(&bufferOutMMC, &bufferInMMC, 1);
	return bufferInMMC;
	//SPIBUF = dat;
...
void XMIT_SPI_MULTI(BYTE *src, UINT cnt) {
	UINT x = 0;
	for (x = 0; x < cnt; x++) {
		xchg_spi(*(src + x));
	}
	//readWriteSPI(src, NULL, cnt);
}

void RCVR_SPI_MULTI(BYTE *dst, UINT cnt) {
	UINT x = 0;
	for (x = 0; x < cnt; x++) {
		*(dst + x) = xchg_spi(0xFF);
	}
	//readWriteSPI(NULL, dst, cnt);
}
